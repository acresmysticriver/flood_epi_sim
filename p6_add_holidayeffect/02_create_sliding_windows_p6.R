...
make_strata <- function(strata_i) {
  ...
  # per week
  end_of_this_period = this_flood$week_num2 + N_WEEKS_PER_PERIOD 
  ...
  case_data <- df_weekly %>%
    filter(year == this_year, 
           week_num2 >= this_flood$week_num2,
           week_num2 < end_of_this_period,
  ...
  while(search_for_prior) {
    ...
    year_prior <- this_year - year_i
    
    start_of_this_period <- this_flood$week_num2 
    end_of_this_period <- start_of_this_period + N_WEEKS_PER_PERIOD 
    
    # Get cut of prior data based on these parameters
    prior_data <- df_weekly %>%
      filter(year == year_prior, 
             week_num2 >= start_of_this_period,
             week_num2 < end_of_this_period,
             county == this_flood$county) %>%
